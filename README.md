...
## What is slskdN?
**[slskdN(OT)](https://github.com/snapetech/slskdn)** (as in, NOT slskd), is a richly feature-added, unofficial fork of slskd. While slskd focuses on being a lean, API-first daemon that lets users implement advanced features via external scripts, **slskdN takes the opposite approach**:
...
- Low-result Wishlist/Lidarr searches retry with up to two bounded term-removal variants when Soulseek returns fewer than 10 peers and files; known operator-suppressed terms (e.g., `Linkin`, `Metallica`) are removed first regardless of position, then generic leading-term removal catches newly-suppressed terms; manual and auto-replace searches stay exact, and mesh receives the raw query
...
- **Suppressed-term recovery** — exact artist/album queries run first; low-result Wishlist searches consult a registry of known operator-suppressed terms (e.g., `Linkin`, `Metallica`) and remove those first, then fall back to bounded leading-term variants; explicit query syntax and mesh searches remain untouched
...
- **Edition matching** — checks each candidate against the track count, duration, and edition of the release Lidarr actually monitors before auto-downloading, so a same-named "Sessions"/live/deluxe edition doesn't get grabbed and rejected by Lidarr's import; strictness is configurable via `integrations.lidarr.edition_match_mode` (`exclude` by default, or `prefer`/`off`)
- **Duplicate-download prevention** — skips a candidate that matches a recently completed download elsewhere by title and duration, so a rejected import that leaves a track "missing" doesn't pile up copies in a different folder on the next sync
