...
| 15 | Add CSL0004 file-path analyzer | Done | (agent) | `TaintToFilePathAnalyzer` flags protocol-tainted `File`, `Directory`, `FileInfo`, `DirectoryInfo`, and `FileStream` path sinks without sanctioned containment validation; analyzer unit tests and the calibration corpus prove the lens fires on known-bad code and stays silent on contained paths. |
| 16 | Add batched semantic sink analyzers | Done | (agent) | `TaintToTimeoutAnalyzer`, `TaintToEndpointAnalyzer`, `TaintToEnumAnalyzer`, and `TaintToStringSliceAnalyzer` flag protocol-tainted delay/timeout, endpoint/URI, enum/status, and slice/index sinks without sanctioned validation; unit tests and the calibration corpus prove each lens fires on known-bad code and stays silent on validated values. |
| 17 | Add full runtime semantic sink batch | Done | (agent) | `TaintToDiagnosticAnalyzer`, `TaintToMessageBuilderAnalyzer`, `TaintToCacheKeyAnalyzer`, `TaintToCryptoTrustAnalyzer`, `TaintToDynamicExecutionAnalyzer`, `TaintToParserRuntimeAnalyzer`, `TaintToResourceCapacityAnalyzer`, and `TaintToBufferOperationAnalyzer` cover the remaining relevant protocol-tainted runtime sink families in one batch; `TaintToLoopBoundAnalyzer` also covers `while` and `do` loops. Unit tests and calibration prove each lens fires on known-bad code and stays silent on validated values. |
