...
| `Silent catch or lossy exception boundaries` | 10 | Open | Empty/noop catches remain a broad lifecycle/cleanup queue. Some are intentional best-effort cleanup; others may hide state corruption or lost diagnostics. | Classify each catch as best-effort cleanup, missing diagnostic, or accepted bug; promote missing-diagnostic shapes into a narrow probe. |
...
| `Remote/user text in diagnostics or HTTP errors` | 185 | Open | Broad privacy/logging queue. Some runtime diagnostics intentionally include remote usernames or local filenames; example Web API console output has different risk and should be reviewed separately. | Split runtime diagnostics from example Web API output; accept only high-confidence sensitive token, full path, raw query, or protocol-secret leaks. |
