...
| --- | --- | --- | --- |
| CSL0001 | TaintToAllocation | High | Network-derived allocation size without a sanctioned validator. Tainted sources: `MessageReader<T>.{ReadInteger, ReadLong}`. Sanctioned validators: `ProtocolCountReader.ReadValidatedCount`, `ProtocolValueValidator.*`, `MessageFrameValidator.Validate*`. |
...
5. Add a `require_pattern` to `scripts/check-remediation-baseline.sh` that asserts the diagnostic ID string appears in source — text gate. Behavior is pinned by the analyzer test.
6. Run `dotnet build src/Soulseek.csproj` and confirm the new lens does not fire on existing runtime source. If it does, decide: accept the finding into a sweep register, or refine the lens.
