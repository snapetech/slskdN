...
| SongID | Identify tracks from URLs/files/text | `feature.SongId` plus songid options/integrations | SongID services | songid endpoints gated by `FeatureGate` | SongID UI | `SongIdService` | `SongIdControllerTests` gate coverage | provider smoke | experimental | gate | Enabled by default but explicitly gated; reduce to runtime capability MVP before marketing. |
