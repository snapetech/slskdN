...
| HardeningValidator | Startup config fail-fast | web/security/diagnostics/metrics | startup validation | none | startup logs | `HardeningValidator` | needs bind matrix tests | config smoke | experimental | needs-test | Fix bind exposure semantics before stable claim. |
