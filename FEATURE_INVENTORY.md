...
| HardeningValidator | Startup config fail-fast | web/security/diagnostics/metrics | startup validation | none | startup logs | `HardeningValidator` | needs startup matrix tests | config smoke | experimental | needs-test | Uses BindExposureAnalyzer for remote reachability; still needs full host matrix tests. |
