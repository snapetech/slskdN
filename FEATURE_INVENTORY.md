...
| DHT peer discovery | BitTorrent DHT discovery | `feature.Dht` plus dht options | DHT services | dht endpoints gated by `FeatureGate` | Network/Mesh UI | DHT rendezvous services | unknown | network smoke | experimental | gate | Enabled by default but explicitly gated; no public bootstrap unless DHT options enable it. |
| Mesh overlay networking | TLS P2P mesh | `feature.Mesh` plus mesh options | mesh services | mesh endpoints gated by `FeatureGate` | Mesh UI/footer | mesh services | unknown | network smoke | experimental | gate | Enabled by default but explicitly gated; needs explicit security posture and live smoke. |
