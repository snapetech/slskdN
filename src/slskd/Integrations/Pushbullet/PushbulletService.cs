...
// <copyright file="PushbulletService.cs" company="slskdN Team">
        ...
        private async Task PushInternalAsync(string title, string body)
        {
            try
            {
                ...
                await Retry.Do(
                    ...
                    onFailure: (attempts, ex) => Log.LogWarning("Failed attempt #{Attempts} to send Pushbullet notification {Title}: {Message}", attempts, title, ex.Message),
