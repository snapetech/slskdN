...
Status: complete for the current reconciliation pass. Swarm visualization is
intentionally tied to active swarm jobs rather than restored as a standalone
route. Top-level route smoke coverage is now in
`src/web/src/components/App.test.jsx`. Compatibility-only and experimental
pages need periodic visibility review when their owning feature areas change.
...
1. Add broader G5 guided-flow productization only when a concrete workflow is
   prioritized and the existing mapped surfaces are insufficient.
