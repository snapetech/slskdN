...
- `Program.cs` still owns process/configuration bootstrapping, but ASP.NET service registration lives in `Bootstrap/WebServiceCollectionExtensions.cs`, the request pipeline lives in `Bootstrap/WebApplicationPipelineExtensions.cs`, application-host wiring lives in `Bootstrap/ApplicationHostServiceCollectionExtensions.cs`, core app services live in `Bootstrap/CoreApplicationServiceCollectionExtensions.cs`, the largest experimental feature graph lives in `Bootstrap/ExperimentalFeatureGraphServiceCollectionExtensions.cs`, and user-data persistence registration lives in `Bootstrap/UserDataServiceCollectionExtensions.cs`; continue splitting the broad experimental module into smaller bounded registrations.
