...
- `Program.cs` still owns the request pipeline and process/configuration bootstrapping, but ASP.NET service registration now lives in `Bootstrap/WebServiceCollectionExtensions.cs`, application-host wiring lives in `Bootstrap/ApplicationHostServiceCollectionExtensions.cs`, core app services live in `Bootstrap/CoreApplicationServiceCollectionExtensions.cs`, the largest experimental feature graph lives in `Bootstrap/ExperimentalFeatureGraphServiceCollectionExtensions.cs`, and user-data persistence registration lives in `Bootstrap/UserDataServiceCollectionExtensions.cs`; continue splitting the broad experimental module into smaller bounded registrations.
