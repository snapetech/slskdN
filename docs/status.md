...
- `Program.cs` still owns app host/web/security bootstrapping, but core app services now live in `Bootstrap/CoreApplicationServiceCollectionExtensions.cs`, the largest experimental feature graph lives in `Bootstrap/ExperimentalFeatureGraphServiceCollectionExtensions.cs`, and user-data persistence registration lives in `Bootstrap/UserDataServiceCollectionExtensions.cs`; continue splitting the broad experimental module into smaller bounded registrations.
