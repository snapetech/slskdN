...
| Pods/native sharing | Pod metadata and membership state | Pod services enabled; Gold Star explicitly opt-in | `feature.Pods` lifecycle gate plus exact-positive `SLSKDN_POD_GOLD_STAR_CLUB_AUTOJOIN=true` control | Gold Star creation, publication, and automatic enrollment are all dormant unless the environment variable is exactly `true`. |
