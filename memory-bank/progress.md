...
[2026-05-13T20:25:00Z] Upstream intake audit: screened all upstream `slskd/slskd` PRs and issues updated since 2025-12-01 (108 PRs, 256 issues) and created `docs/dev/upstream-intake-action-notes-2026-05-13.md` with slskdN-oriented action buckets, release-blocking candidates, runtime fork watchlist items, and a full source inventory. No upstream write actions were taken.

[2026-05-13T20:45:00Z] Upstream intake hardening pass: reconciled the action buckets against current slskdN implementation, hardened peer/mesh stream ticket validation for traversal/rooted filenames and mesh SHA-256 hash expectations, added config compatibility warning regression tests, and verified already-present transfer/search/upload/group/blacklist/Lidarr-Wishlist coverage with focused tests. Dependency vulnerability checks reported no current NuGet or production npm vulnerabilities.
